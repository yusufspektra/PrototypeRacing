...
namespace UniLabs.Time
{
    public static class TimeDrawerUtils
    {
        ...
        public static TimeSpan DrawTimeSpanField(GUIContent label, TimeSpan currentValue,
            TimeUnit highestUnit = TimeUnit.Days, TimeUnit lowestUnit = TimeUnit.Seconds)
        {
        ...
        private static TimeSpan DrawTimeUnitField(TimeSpan currentValue, TimeUnit unit,
            TimeUnitExtensions.GetUnitValueDelegate getter, TimeUnitExtensions.WithUnitValueDelegate setter)
        {
            ...
            GUI.Label(GUILayoutUtility.GetLastRect().HorizontalPadding(0.0f, 8f), prefixLabel,
        ...
        private static TimeSpan DrawTimeUnitLowestField(TimeSpan currentValue, TimeUnit unit) =>
            DrawTimeUnitField(currentValue, unit, TimeUnitExtensions.GetLowestUnitValue,
                TimeUnitExtensions.WithLowestUnitValue);

        private static TimeSpan DrawTimeUnitHighestField(TimeSpan currentValue, TimeUnit unit) =>
            DrawTimeUnitField(currentValue, unit, TimeUnitExtensions.GetHighestUnitValue,
                TimeUnitExtensions.WithHighestUnitValue);

        private static TimeSpan DrawTimeUnitSingleField(TimeSpan currentValue, TimeUnit unit) =>
            DrawTimeUnitField(currentValue, unit, TimeUnitExtensions.GetSingleUnitValue,
                TimeUnitExtensions.FromSingleUnitValue);

        public static TimeSpan DrawTimeSpanSlider(TimeSpan value, TimeSpan min, TimeSpan max, TimeUnit snapping)
        {
            var rect = EditorGUILayout.GetControlRect(false);
            var totalSeconds = GUI.HorizontalSlider(rect, (float)value.TotalSeconds, (float)min.TotalSeconds,
                (float)max.TotalSeconds);
            return TimeSpan.FromSeconds(totalSeconds).SnapToUnit(snapping);
        }

        public static Tuple<TimeSpan, TimeSpan> DrawTimeSpanMinMaxSlider(TimeSpan start, TimeSpan end, TimeSpan min,
            TimeSpan max, TimeUnit snapping)
        {
            var startSeconds = (float)start.TotalSeconds;
            var endSeconds = (float)end.TotalSeconds;

            EditorGUILayout.MinMaxSlider(ref startSeconds, ref endSeconds, (float)min.TotalSeconds,
                (float)max.TotalSeconds);
        ...
        public static void DrawDateTimeField(GUIContent label, DateTime currentValue, Action<DateTime> valueSetter)
        {
            ...
            EditorGUILayout.BeginHorizontal();
            {
                ...
                {
                    if (label == null)
                        label = new GUIContent("no_name");
                    
                    EditorGUILayout.TextField(label, currentValue.ToString("dd.MM.yyyy HH:mm:ss.fff"));
                }
                EditorGUI.EndDisabledGroup();

                var rect = GUILayoutUtility.GetRect(18, 18, SirenixGUIStyles.Button,
                    GUILayoutOptions.ExpandWidth(false).Width(18));
                if (SirenixEditorGUI.IconButton(rect, EditorIcons.DayCalendar))
                {
                    var dateTimePicker =
                        new DateTimePicker(currentValue, dateTime => { valueSetter?.Invoke(dateTime); });
